...
class Index extends CI_Controller {
	...
	{
		$data['do_donation']=$this->index_model->getCards('donation',array('status'=>1),'transaction.qty,donation_type.image,users.pro_img,users.first_name,users.last_name,donation.qty as goal_qty,donation.name as donation_title,donation_type.name as donation_name,donation.id as donation_id',array('donation.datetime'=>'ASC'),false,8);
		$data['reqst_donation']=$this->index_model->getCards('donation',array('status'=>0),'transaction.qty,donation_type.image,users.pro_img,users.first_name,users.last_name,donation.qty as goal_qty,donation.name as donation_title,donation_type.name as donation_name,donation.id as donation_id',array('donation.datetime'=>'ASC'),false,4);
