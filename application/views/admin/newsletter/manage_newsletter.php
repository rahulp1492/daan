...
<div id="main-content">
    ...
    <div class="page-title">
        <div>
            <h1><i class="fa fa-newspaper-o"></i> <?php echo isset($module_name)? $module_name:'Demo Page'; ?></h1>
            ...
            <div class="box-content">
                <div class="btn-toolbar pull-right">
                    <div class="btn-group">
                        ...
                        <a class="btn btn-circle show-tooltip" title="Refresh" href="<?php echo base_url('/admin/newsletter');?>"><i class="fa fa-refresh"></i></a>
                    </div>
                </div>
                <br><br>
              ...
              <form name="frm_make_view" id="frm_make_view" method="post" action="<?php echo base_url().ADMIN_CTRL;?>/newsletter/multi_action">
                <?php  $csrf = array(
                    ...
                    <tbody>
                        <?php if($total>0){
                            ...
                            <tr class="">
                              ...
                              <td>
                                <?php if($record['subscribe_status']==1){ ?>
                                <a class="show-tooltip" title="Active" onclick="return confirm('Are you sure you want to block record ?')" href="<?php echo base_url('/admin/newsletter/unsubscribe/').base64_encode($record['id']) ?>"><i class="fa fa-unlock"></i></a>
                                <?php }else{ ?>
                                <a class="show-tooltip" onclick="return confirm('Are you sure you want to unblock record ?')" title="Inactive" href="<?php echo base_url('/admin/newsletter/subscribe/').base64_encode($record['id']) ?>"><i class="fa fa-lock"></i></a>
                                <?php } ?>
                            </td>
                            <td class="visible-md visible-lg">
                                <div class="btn-group">

                                    <a class="btn btn-sm btn-danger show-tooltip" onclick="return confirm('Are you sure you want to delete record ?')" title="" href="<?php echo base_url('/admin/newsletter/delete/').base64_encode($record['id']); ?>" data-original-title="Delete"><i class="fa fa-trash-o"></i></a>
                                </div>
                            </td>
                        </tr>
                        <?php
...
function unblockselect()
{
    ...
    {
        if(confirm("Do You Want To Unsubscribe This Record ?")==true)
        {
            $("#action").val('unblock');
            $("#frm_make_view")[0].submit();
        }
...
else
{
    if(chk.checked)
    {
        if(confirm("Do You Want To Subscribe This Record ?")==true)
        {
            $("#action").val('unblock');
            $("#frm_make_view")[0].submit();
        }
